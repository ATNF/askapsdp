...
class MELinearSolverTest : public CppUnit::TestFixture  {
  ...
  public:
    void setUp()
    {
      ...
      for (uint chan=0;chan<nChan;chan++) ida->mFrequency(chan)=1.400e9-2e6*chan;
      uint nPol(1);
	...
	void testSVD() {
		...
		CPPUNIT_ASSERT(abs(q.cond()-1.69147e+06)<100.0);
	    solver1.parameters().fix("direction.dec.cena");
		solver1.solveDesignMatrix(q);
		CPPUNIT_ASSERT(q.rank()==2);
		CPPUNIT_ASSERT(abs(q.cond()-1.53092e+06)<100.0);
