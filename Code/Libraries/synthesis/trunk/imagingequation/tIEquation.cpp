...
int main() {
	...
	// The data source
	MSIEqDataSource msds(string("xntd.ms"));
...
//	gsmparams.add(string("gsm.cl"));
	GSMIEquation gsm(string("Global Sky Model Equation"), gsmparams);
...
//	cenaparams.add(string("cena.image"));
	cenaparams.add(string("EJones.gain"), IEqParam(0.0));
	PeelingIEquation cena(string("Peeling Equation"), cenaparams);
...
//	cmosaicparams.add(string("cmosaic.image"));
//	cmosaic.setContext(IEquation::HIGHDYNAMICRANGE);
	MosaicIEquation cmosaic(string("Continuum Mosaic Equation"), cmosaicparams);
...
//	smosaic.setContext(IEquation::LOWDYNAMICRANGE);
	MosaicIEquation smosaic(string("Spectral line Mosaic Equation"), smosaicparams);
	...
	for (uint iteration=0;iteration<3;iteration++) {
		...
		// have minimal effect
		ImageIEqSolver is(string("Image solver"), cena.parameters());
		is.init();
		...
		ImageIEqSolver cis(string("Image solver"), cmosaic.parameters());
		cis.init();
		msds.init();
		while (msds.next()) {
			msds.ida().initmodel();
		...
		ImageIEqSolver sis(string("Image solver"), smosaic.parameters());
		sis.init();
		msds.init();
		while (msds.next()) {
			msds.ida().initmodel();
	...
	cena.parameters().saveAsTable(string("cena.peeling"));
