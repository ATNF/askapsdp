...
namespace askap {
	...
	void slidingBoxStats(casa::Array<Float> &input, casa::Array<Float> &middle, casa::Array<Float> &spread, casa::IPosition &box, bool useRobust)
	{
	    ...
	    if(useRobust) {
		middle = slidingArrayMath(input, box, MedianFunc<Float>(False,True,False));
		spread = fabs(input - middle);
		spread = slidingArrayMath(spread, box, MedianFunc<Float>(False,True,True)) / Statistics::correctionFactor;
	    }
	    else {
