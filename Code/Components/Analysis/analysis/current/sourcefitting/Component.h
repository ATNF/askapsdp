...
class SubComponent {
    public:
        ...
        /// the peak flux only, with a strict less-than comparison.
        friend bool operator< (SubComponent lhs, SubComponent rhs);

        /// @brief Stream output function.
        /// @details Output the key parameter values to the stream. The
        /// flux has a precision of 8 and the rest a precision of 3.
