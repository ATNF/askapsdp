...
const boost::shared_ptr<ImageInterface<Float> > openImage(std::string imagename)
{
    ...
    const boost::shared_ptr<LatticeBase> lattPtr(ImageOpener::openImage(imagename));

    if (lattPtr.get() == 0)
        ASKAPTHROW(AskapError, "Requested image \"" << imagename <<
...
const boost::shared_ptr<SubImage<Float> > getSubImage(std::string imagename, Slicer slicer)
{
    ...
    const boost::shared_ptr<SubImage<Float> > sub(new SubImage<Float>(*imagePtr, slicer));
    return sub;
}
    ...
    if (wcs->spec >= 0) { //if there is a spectral axis
        ...
        if (shortType == "VELO" || shortType == "VOPT" || shortType == "ZOPT" ||
            ...
            if (wcs->restfrq != 0) {
                ...
                // No rest frequency defined, so put spectral
                // dimension in frequency.  Set the spectral axis to a
                // standard specification: FREQ
                DUCHAMPWARN("Cube Reader",
        ...
        // Now we need to make sure the spectral axis has the correct
        //  setup.  We use wcssptr to translate it if it is not of the
        //  desired type, or if the spectral units are not defined.
...
std::string getFullSection(std::string filename)
{
    ...
    const boost::shared_ptr<ImageInterface<Float> > imagePtr = openImage(filename);
...
float findSurroundingNoise(std::string filename, float xpt, float ypt, int noiseBoxSize)
{
    ...
    ASKAPLOG_DEBUG_STR(logger, "Found noise of " << madfm << " in box size " <<
                       noiseBoxSize << " centred on " << xpt << "," << ypt
                       << " in image " << filename << " using start=" <<
                       start << " and end=" << end);
...
wcsprm *casaImageToWCS(std::string imageName)
{
    ...
    /// to access it. Calls casaImageToWCS(const boost::shared_ptr<ImageInterface<Float> >).
...
wcsprm *casaImageToWCS(const boost::shared_ptr<ImageInterface<Float> > imagePtr)
{
    ...
    /// the image.
    IPosition shape = imagePtr->shape();
    std::vector<int> dim = shape.asStdVector();

    CoordinateSystem coords = imagePtr->coordinates();
    Record hdr;

    if (!coords.toFITSHeader(hdr, shape, true, 'c', true, true, true, false))
        throw AskapError("casaImageToWCS: could not read FITS header parameters");
    ...
    if (status)
        ASKAPTHROW(AskapError, "casaImageToWCS: wcsini failed! Code=" <<
                   status << ": " << wcs_errmsg[status]);

    status = wcsset(wcs);

    if (status) {
        ASKAPTHROW(AskapError, "casaImageToWCS: wcsset failed! WCSLIB error code=" <<
                   status  << ": " << wcs_errmsg[status]);
    }
    ...
    status = wcsfix(1, dim.data(), wcs, stat);
...
void fixSlicer(Slicer &slice, wcsprm *wcs)
{
    ...
    if (start >= end) {
        ASKAPLOG_FATAL_STR(logger, "ERROR with fixSlicer: start=" << start <<
                           ", end=" << end << ", stride=" << stride <<
                           ", caused by Slicer " << slice <<
                           " and WCS indices (" << wcs->lng << "," <<
                           wcs->lat << "," << wcs->spec << ")");
    }
...
void lengthenSlicer(Slicer &slice, int ndim)
{
    ...
    if (oldDim < ndim) {
        ...
        if (start >= end) {
            ASKAPLOG_FATAL_STR(logger, "ERROR with lengthenSlicer: start=" << start <<
                               ", end=" << end << ", stride=" << stride <<
                               ", caused by Slicer " << slice <<
                               " going from " << oldDim << "dim to " << ndim << "dim");
        }
