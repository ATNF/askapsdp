...
public class JDBCPersistenceInterface {
	...
	private static Logger logger = Logger
			.getLogger(JDBCPersistenceInterface.class.getName());
	 ...
	 * Constructor
	 */
	public JDBCPersistenceInterface(String url, String userName, String password) {
	 ...
	 * 
	 * @param ra
	 *            the right ascension of the centre of the search area (Units:
	 *            decimal degrees). Range: 0 to 359.9...
	 * @param dec
	 *            the declination of the centre of the search area (Units:
	 *            decimal degrees). Range: +/- 90
	 * @param searchRadius
	 *            the search radius (Units: decimal degrees).
	 * @param fluxLimit
	 *            low limit on flux on sources returned. All returned sources
	 *            shall have flux >= fluxLimit (Units: Jy).
	...
	public List<Long> coneSearch(double ra, double dec, double searchRadius,
		...
		final String query = "SELECT id,right_ascension,declination FROM components WHERE (i1400 >="
				+ fluxLimit
				+ ") AND (declination >= "
				+ (dec - searchRadius)
		...
		ResultSet rs = null;
		try {
			s = itsConn.createStatement(java.sql.ResultSet.TYPE_FORWARD_ONLY,
			...
			try {
				...
				// Can't do anything except report it
	...
	/**
	 * @param componentIds
	 *            a list of component IDs for which the component objects they
	 *            identify are to be returned.
	...
	public List<Component> getComponents(final List<Long> componentIds) {
		...
		ResultSet rs = null;
		try {
			s = itsConn.prepareStatement(query, ResultSet.TYPE_FORWARD_ONLY,
	...
	/**
	 * @param components
	 *            a list of components to add to the sky model.
	...
	/**
	 * @param componentIds
	 *            a vector of component IDs for which those components are to be
	 *            removed from the sky model.
	 */
	public void removeComponents(List<Long> componentIds) {
	}

	/**
	 * Calculates the angular separation between a position given by ra/dec and
	 * a component instance.
	 * 
	 * @param ra
	 *            right ascension in decimal degrees (Range: 0 to 359.9...)
	 * @param dec
	 *            declination in decimal degrees (Range: +/-90)
	 * @param comp
	 *            component to determine angular separation for
	 * @return the angular separation between the position specified by the
	 *         parameters ra and dec, and the specified component.
	 */
	static private double angularSeparation(double ra, final double dec,
			final Component comp) {
		final double[] refXYZ = createXYZ(Math.toRadians(ra),
				Math.toRadians(dec));
	 ...
	 */
	static private double square(final double val) {
		return Math.pow(val, 2);
	}

	/**
	 * For a given ra/dec creates a 3-vector of direction cosines, the Cartesian
	 * coordinates for the given position.
	 * 
	 * @param ra
	 *            right ascension in decimal degrees (Range: 0 to 359.9...)
	 * @param dec
	 *            declination in decimal degrees (Range: +/-90)
	 * @return the Cartesian coordinates, x, y & z. Where x points to the prime
	 *         meridian equator, z points to the north pole, and y is normal to
	 *         x and z.
	 */
