    ...
    itsBaselineMap(parset)
{
    ...
    ASKAPCHECK(fileNames.size() == itsBaselineMap.size(),
               "Number of flag files given in the parset (" << fileNames.size()
               << ") is expected to match the number of defined baseline/polarisation products ("
               << itsBaselineMap.size() << ")");
    ...
    for (size_t i = 0; i < fileNames.size(); ++i) {
        ...
        while (is) {
            ...
            is >> buf;
            if (is) {
                ASKAPCHECK((buf >= 0) && (buf < 16416),
                           "Each channel number is expected to be between 0 and 16415 inclusive");
                itsChannelsToFlag[i].push_back(size_t(buf));
            }
        }
        ASKAPLOG_INFO_STR(logger, "    will flag " << itsChannelsToFlag[i].size()
                << " spectral channels");
    }
}
                                 ...
                                 const casa::uInt baseline, const casa::uInt)
{
    ...
    for (std::vector<size_t>::const_iterator ci = itsChannelsToFlag[baseline].begin(); ci != endMark; ++ci) {
        ASKAPCHECK(*ci < vis.nelements(), "Encountered channel " << *ci
