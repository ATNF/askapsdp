...
void SimpleMonitorTask::process(askap::cp::common::VisChunk::ShPtr chunk)
{
    ...
    if (chunk->nChannel() >= 2) {
        ...
        ASKAPCHECK(abs(resolution) > 0., "Zero frequency increment has been encountered for time: "
                   << chunk->time());
...
void SimpleMonitorTask::publishBuffer()
{
    if (!itsOStream.is_open()) {
        try {
            ...
            ASKAPLOG_FATAL_STR(logger, "Error opening output ascii file for monitoring information: "
                               << ex.what());
        }
    }
    // time is in minutes
