/// @file UVChannelDataSource.cc
/// @brief Implementation of IDataSource
///
...
UVChannelDataSource::UVChannelDataSource(const LOFAR::ParameterSet& parset,
        const std::string& channelName)
        : UVChannelConstDataSource(parset, channelName)
{
}
    ...
    const IDataConverterPtr &conv) const
{
    ...
    if (!implSel || !implConv) {
        ASKAPTHROW(DataAccessLogicError, "Incompatible selector and/or " <<
                   "converter was passed to the createIterator method");
    }

    return boost::shared_ptr<IDataIterator>(new UVChannelDataIterator(
