#
# AIPS++ script to simulate MEERKAT observations at 1.5GHz
#
#
# Parameters
#
...
note('***** MeerKAT Sim Observing details *****');
...
#
  vp:=vpmanager();
  print vp.setpbairy('MEERKAT', dopb=T, dishdiam='12m', blockagediam='1m', maxrad='10deg');
...
#
  print mysim.setconfig('MEERKAT', x=rec.x[1:nant], y=rec.y[1:nant], z=rec.z[1:nant], 
...
#
  for (feed in 1:nfeeds) {
    ...
    mysim.setfield(spaste('MEERKAT_FEED_', feed),
...
#    
  ...
  for (step in 1:nstep) {
    for (feed in 1:nfeeds) {
      print "Observing ", spaste('MEERKAT_FEED_', feed), spaste('LBAND',spwid), spaste(tstart, 's'), spaste(tstart+nint*t, 's');
      if(!mysim.observe(spaste('MEERKAT_FEED_', feed), 
			spaste('LBAND',spwid),
