#
# Define MeerKAT configuration for use in AIPS++ simulator
#
readconf:=function(filename='meerkat80.txt') {

  include 'measures.g';

  rec:=[=];
  rec.location:=dm.position('WGS84', '+21.388374deg', '-30.714951deg');
  ...
  for (i in 1:nant) {
    rec.name[i]:=sprintf('MEERKAT%02d', i);
  }
  return rec;
}

plotconf:=function(filename,baseline=10000) {
  include 'pgplotter.g';
  ...
  p.setplottitle(spaste('MeerKAT configuration: ', filename));
  p.setxaxislabel('X (m)');
  p.setyaxislabel('Y (m)');
  p.iden();
  p.sci(2);
