...
void sendNE(MPIConnectionSet::ShPtr& cs, int nnode, int rank,
    const NormalEquations& ne)
{
	...
	cs->write(0, bs);
	os() << "PREDIFFER "<< rank
	    << " Sent normal equations to the solver via MPI in "<< timer.real()
	    << " seconds "<< std::endl;
}
...
void sendModel(MPIConnectionSet::ShPtr& cs, int nnode, const Params& skymodel)
{
	...
	os() << "SOLVER Sent model to the prediffers via MPI in "<< timer.real()
	    << " seconds "<< std::endl;
}

// Receive the model from the solver
void receiveModel(MPIConnectionSet::ShPtr& cs, int rank, int nnode,
    Params& skymodel)
{
	if (nnode==1)
		return;
...
void solveNE(Params& skymodel, Solver::ShPtr& solver)
{
	...
	os() << "SOLVER Solved normal equations in "<< timer.real() << " seconds "
	    << std::endl;
...
void summariseModel(Params& skymodel)
{
	...
	for (vector<string>::iterator it=resultimages.begin(); it!=resultimages.end(); it++)
	{
		...
		os() << *it << std::endl<< "Maximum = "<< max(resultImage)
		    << ", minimum = "<< min(resultImage) << std::endl;
	}
}
    ...
    const casa::Vector<casa::Quantum<double> > qbeam)
{
	...
	if (restore)
	{
		...
		ir.setThreshold(solver->threshold());
		ir.setVerbose(solver->verbose());
