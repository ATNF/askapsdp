...
/// Data to grid (MFS)
      ...
      bool dopsf)
    {
      ...
      casa::Cube<float> visweight(idi->visibility().shape()); visweight.set(1.0f);
...
// visibility to the grid.
    ...
    /// Data to grid (MFS)
    void TableVisGridder::genericReverseWeights(const casa::Cube<float>& visweight,
      casa::Matrix<double>& sumWeights)
    {
