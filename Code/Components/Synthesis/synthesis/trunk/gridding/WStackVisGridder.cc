			...
			for (int i=0; i<nSamples; i++)
			{
				...
				for (int chan=0; chan<nChan; chan++)
				{
					for (int pol=0; pol<nPol; pol++)
					{
						...
						CONRADCHECK(itsGMap(i,pol,chan)<itsNWPlanes, "W scaling error: recommend allowing larger range of w");
						CONRADCHECK(itsGMap(i,pol,chan)>-1, "W scaling error: recommend allowing larger range of w");
					}
				}
			}
		}
