			...
			for (int i=0; i<nSamples; i++) {
				...
				for (int chan=0; chan<nChan; chan++) {
					for (int pol=0; pol<nPol; pol++) {
						...
						/// Order is (chan, feed)
						if (itsFreqDep) {
							itsCMap(i, pol, chan)=chan+nChan*(feed+itsMaxFeeds*itsLastField);
							ASKAPCHECK(itsCMap(i, 0, chan)<itsMaxFields*itsMaxFeeds
			...
			for (int row=0; row<nSamples; row++) {
				...
				if (newField&&!itsDone(feed, itsLastField)) {
					...
					*cos(offset.getLong()-out.getLong());
					...
					double ax=2.0f*casa::C::pi*cellu*itsSlopes(0, feed, itsLastField);
					double ay=2.0f*casa::C::pi*cellv*itsSlopes(1, feed, itsLastField);
					...
					for (int chan=0; chan<nChan; chan++) {
						...
						if (itsSupport==0) {
							...
							itsConvFunc.resize(itsOverSample*itsOverSample*itsMaxFeeds
									*nChan);
							itsSumWeights.resize(itsMaxFeeds*nChan, itsShape(2),
									itsShape(3));
							itsSumWeights.set(casa::Complex(0.0));
						}
						/// Calculate the index into the convolution functions
						/// Order is (chan, feed)
						zIndex=chan+nChan*(feed+itsMaxFeeds*itsLastField);
